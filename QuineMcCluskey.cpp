...
void QuineMcCluskey::readInput(const string &filename)
{
    ...
    {
        cerr << "Error: Missing number of variables line." << endl;
        run = false;
        return;
    }
    ...
    try {
        ...
        if (numVariables > 20) {
            cerr << "Error: Number of variables is out of range." << endl;
            run = false;
            return;
        }
    } catch (...) {
    ...
    int maxVal = pow(2, numVariables);

    // Read second line (minterms or maxterms)
    if (!getline(file, text))
    {
        cerr << "Error: Missing minterms/maxterms line." << endl;
        run = false;
        return;
    }
    ...
    {
        cerr << "Error: Second line must start with 'm' or 'M' to indicate minterms or maxterms." << endl;
        run = false;
        return;
    }
    ...
    string term;
    vector<int> MaxtermTemp;
    set<int> minOrMaxVals;
    bool MaxCase = false;
    int value;
    int termCount = 0;
    ...
    while (getline(ss, term, ','))
    {
        ...
        if (term.size() > 1)
        {
            if (term[0] == 'M')
            {
                MaxCase = true;
                if (isdigit(term[1])) {
                    value = stoi(term.substr(1));
                    if (value >= maxVal) {
                        cerr << "Error: Maxterm at position " << termCount << " is out of range for " << numVariables << " variables." << endl;
                        run = false;
                        return;
                    }
                    MaxtermTemp.push_back(value);
                    minOrMaxVals.insert(value);
                } else {
                    cerr << "Error: Invalid maxterm format at position " << termCount << ": " << term << endl;
                    run = false;
                    return;
                }
            }
            else if (term[0] == 'm')
            {
                if (isdigit(term[1])) {
                    value = stoi(term.substr(1));
                    if (value >= maxVal) {
                        cerr << "Error: Minterm at position " << termCount << " is out of range for " << numVariables << " variables." << endl;
                        run = false;
                        return;
                    }
                    terms.push_back(Term(value, false));
                    minOrMaxVals.insert(value);
                } else {
                    cerr << "Error: Invalid minterm format at position " << termCount << ": " << term << endl;
                    run = false;
                    return;
                }
            }
            else {
                cerr << "Error: Invalid term prefix at position " << termCount << ": " << term << endl;
                run = false;
                return;
            }
        }
        else if (term.size() == 1 && term[0] == 'M')
        {
            MaxCase = true;
            terms.clear();
            for (int i = 0; i < maxVal; i++) {
                terms.push_back(Term(i, false));
                minOrMaxVals.insert(i);
            }
        }
        else {
            cerr << "Error: Malformed term at position " << termCount << ": " << term << endl;
            run = false;
            return;
        }
    }
    ...
    // Read third line (don't cares)
    if (!getline(file, text))
    {
        cerr << "Error: Missing don’t care terms line." << endl;
        run = false;
        return;
    }
    ...
    {
        cerr << "Error: Don’t care line must start with 'd'." << endl;
        run = false;
        return;
    }
    ...
    while (getline(ss, term, ','))
    {
        ...
        if (term.size() > 1 && term[0] == 'd' && isdigit(term[1]))
        {
            value = stoi(term.substr(1));
            if (value >= maxVal) {
                cerr << "Error: Don’t care value at position " << termCount << " is out of range for " << numVariables << " variables." << endl;
                run = false;
                return;
            }
            if (minOrMaxVals.find(value) != minOrMaxVals.end()) {
                cerr << "Error: Value " << value << " at position " << termCount << " appears in both minterms/maxterms and don’t-cares." << endl;
                run = false;
                return;
            }
            terms.push_back(Term(value, true));
        }
        else if (!term.empty() && term[0] != 'd')
        {
            cerr << "Error: Invalid don’t care term format at position " << termCount << ": " << term << endl;
            run = false;
            return;
        }
    }
